...
# Rename columns to have a common 'Song_ID' for merging
spotify_songs = spotify_songs.rename(columns={'track_id': 'Song_ID', 'track_popularity': 'Popularity'})

# Ensure that 'User ID' is properly named for merging
...
def recommend_music(user_profile_index, num_recommendations=5):
    try:
        user_profile_index = int(user_profile_index)
        if user_profile_index < 0 or user_profile_index >= len(merged_df):
            raise ValueError(f"User index '{user_profile_index}' is out of range.")

        sim_scores = list(enumerate(cosine_sim[user_profile_index]))
        sim_scores = sorted(sim_scores, key=lambda x: x[1], reverse=True)
        recommended_indices = [i[0] for i in sim_scores[1:num_recommendations + 1]]

        return merged_df.iloc[recommended_indices][['Song_ID']].values.flatten().tolist()
    except ValueError as e:
        return str(e)

# Hybrid Recommendation Function
def get_hybrid_recommendations(user_id, user_profile_index, cf_weight=0.5, cbf_weight=0.5, top_n=5):
    cf_recommendations = get_cf_recommendations(user_id, top_n * 2)
    cbf_recommendations = recommend_music(user_profile_index, top_n * 2)
...
if __name__ == "__main__":
    while True:
        ...
        user_profile_index = input("Enter the User Profile Index: ").strip()

        top_recommendations = get_hybrid_recommendations(user_id, user_profile_index)
